...
function mobileView() {
  return `<section class="mobile-panel"><div class="mobile-panel__hero"><span>INVOICEFLOW MOBILE</span><h2>我的发票与回款</h2><p>移动端与财务 API 共享开票申请、回款登记、余额核销和跟进提醒。</p><button class="primary" data-action="create-appointment">＋ 创建演示发票</button></div><div class="mobile-list"><h3>今日发票</h3>${appointments.slice(0, 4).map((appointment) => `<article class="mobile-card"><div><small>${timeLabel(appointment.scheduledAt)} · ${appointment.department}</small><strong>${appointment.patient}</strong><span>${appointment.doctor} · ${appointment.status}</span></div><b class="status ${statusColors[appointment.status] || 'indigo'}">${appointment.status}</b>${appointmentAction(appointment)}</article>`).join('')}</div><div class="mobile-list"><h3>我的回款跟进</h3>${followupTasks.slice(0, 3).map((item) => `<article class="mobile-card"><div><small>${item.dueAt}</small><strong>${item.summary}</strong><span>${item.patient} · ${item.status}</span></div>${item.status === '已完成' ? '<b class="status green">已完成</b>' : `<button class="text-action" data-action="complete-followup" data-followup-id="${item.id}">完成跟进</button>`}</article>`).join('')}</div></section>`
}
