...
test('InvoiceFlow has invoice, finance schedule and collection task data', async()=>{const source=await readFile(new URL('../src/main.js',import.meta.url),'utf8'); assert.match(source,/今日发票队列/); assert.match(source,/财务专员排班/); assert.match(source,/跟进任务/); assert.match(source,/INV-0716-082/)})
