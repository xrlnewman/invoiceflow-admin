...
test('InvoiceFlow has appointment, schedule and followup data', async()=>{const source=await readFile(new URL('../src/main.js',import.meta.url),'utf8'); assert.match(source,/今日发票队列/); assert.match(source,/负责人排班/); assert.match(source,/回访任务/); assert.match(source,/AP-0716-082/)})
