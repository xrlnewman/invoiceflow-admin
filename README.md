...
InvoiceFlow 是免费开源的发票收款运营运营后台，覆盖发票队列、负责人排班、客户档案和回访任务。它只做运营流程，不做诊断、处方、支付或真实客户数据。
...
后台的“发票队列”和“回访任务”按钮会优先调用真实 API；API 暂不可用时保留内置演示数据并提示当前数据来源。侧栏“移动端体验”提供同一闭环的窄屏客户视图，支持创建演示发票、确认、候诊、处理完成和回访完成，便于用手机浏览器联调。
...
InvoiceFlow 是免费开源的 发票收款样板，覆盖创建、分派、处理、完成、回访的完整闭环。所有演示数据均为虚构，不接入真实财务、人事或客户隐私。
